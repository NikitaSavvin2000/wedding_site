...
export function VenueSection() {
  const { containerMaxWidth, imageHeight, cardPadding } = THEME_CONFIG.venueCards;
  ...
  const renderVenueCard = (venueItem: any, labelKey: string, imageSrc: string) => (
    ...
    >
      <div className="relative overflow-hidden" style={{ height: imageHeight.mobile }}>
        <ImageWithFallback
          src={imageSrc}
          alt={venueItem.name}
      ...
      <div className={cardPadding.mobile}>
        ...
        <div className="space-y-4">
          <div
            ...
            style={{ fontFamily: 'Montserrat, sans-serif', color: '#6B7456', fontWeight: '300' }}
          >
            <Clock className="w-7 h-7 mt-0.5 flex-shrink-0" style={{ color: '#CDD4B1' }} />
            <span>{venueItem.time}</span>
          </div>

          <div
            onClick={() => copyToClipboard(venueItem.address, labelKey)}
            className="flex items-center justify-between gap-3 text-xl cursor-pointer group"
            style={{ fontFamily: 'Montserrat, sans-serif', color: '#6B7456', fontWeight: '300' }}
          >
            <div className="flex items-start gap-3">
              <MapPin className="w-7 h-7 mt-0.5 flex-shrink-0" style={{ color: '#CDD4B1' }} />
            ...
            <div className="flex-shrink-0 mt-1 opacity-50 group-hover:opacity-100 transition">
              {copiedAddress === labelKey ? (
                <Check className="w-5 h-5" style={{ color: '#8B9474' }} />
              ) : (
                <Copy className="w-5 h-5" style={{ color: '#CDD4B1' }} />
              )}
            </div>
          </div>
        </div>
  ...
  return (
    <section className="py-24 px-6" style={{ backgroundColor: '#FFF9E2' }}>
